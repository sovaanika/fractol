...
/*   Updated: 2019/03/25 20:14:12 by bbear            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	julia(t_fract *fract)
{
	...
	while (++x < fract->wid)
	{
		...
		while (++y < fract->hei)
		{
			fract->frac.newre = (x - fract->wid / 2) / (0.45 * fract->wid * fract->zoom) + fract->movex;
			fract->frac.newim = (y - fract->hei / 2) / (0.45 * fract->hei * fract->zoom) + fract->movey;
			i = -1;
			...
			//i = abs((int)n);
			fract->data[x + y * fract->wid] = (i * 7) % 255 * 256 * 256 + (i * 7) % 255 * 256 + (i * 7) % 255;
		}
	}
