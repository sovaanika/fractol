...
/*   Updated: 2019/03/21 19:00:58 by bbear            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++x < fract->wid)
	{
		...
		while (++y < fract->hei)
		{
			...
			while (++i < fract->maxiter)
			{
				oldre = newre;
				oldim = newim;
				newre = (oldre * oldre - oldim * oldim + (x - 0.5 * fract->wid) / (0.25 * fract->wid)) * fract->zoom;
				newim = (2 * oldre * oldim + (y - 0.5 * fract->hei) / (0.25 * fract->hei)) * fract->zoom;
